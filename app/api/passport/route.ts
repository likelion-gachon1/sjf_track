// POST /api/passport — 여권 멘트 생성 (서버 전용).
// 반환 { travelType: "CULTURE NOMAD", reason: "코냑이 스며든 단단한 걸음" }.
// 실패하면 5xx 로 응답하고 클라이언트가 lib/passport.ts 의 폴백으로 대체합니다.
//
// ⚠️ OPENAI_API_KEY 에 NEXT_PUBLIC_ 접두사를 붙이지 마세요 — 브라우저 번들에 노출됩니다.
...
// 제약(길이·어미) 있는 한국어 창작이라 분류용 Luna 보다 상위 티어를 씁니다.
//
// ⚠️ gpt-5.6 계열이 거부하는 파라미터 2종 (둘 다 400):
//    - max_tokens  → max_completion_tokens 로 이름이 바뀜
//    - temperature → 기본값 1 만 허용. 환각은 프롬프트로 막습니다.
...
/**
 * 카피 다듬기. 길이는 **자르지 않습니다** — 한국어를 글자 수로 자르면 단어 중간이
 * 끊기므로 길이는 프롬프트에 맡기고, 여기서는 톤을 깨는 문장부호만 뗍니다.
 */
...
export async function POST(req: Request) {
  ...
  const system = [
    ...
    // ⚠️ 재료 나열을 막는 문단. "엮으라"고 하면 모델이 슬롯 이어붙이기로 읽습니다.
    ...
    // ⚠️ 지우지 마세요 — 없으면 입력에 없는 색을 지어냅니다(7회 중 1회 실측).
    "카피에 쓸 수 있는 색은 **위에 주어진 제품 컬러와 착장 컬러뿐**입니다. 주어지지 않은 색 이름을 새로 지어내지 마세요.",
    // ⚠️ 위 규칙의 부작용 차단 — 없으면 제품색과 착장색을 한 문장에 같이 넣어 나열이 됩니다.
    ...
    // ⚠️ 길이·어미 규칙은 반드시 마지막에 — 앞에 두면 재료에 밀려 무시됩니다.
    //    하한을 지우면 "강렬한 블랙의 시크함"(11자)처럼 앙상하게 끊습니다.
    ...
    // ⚠️ 나쁜 예는 실제로 나왔던 출력입니다. 추상적인 금지보다 훨씬 잘 먹히니 두세요.
