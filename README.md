...
`font-serif` 도 같은 서체를 가리키므로 기존 `font-serif` 제목들도 함께 적용됩니다.
굵기는 Tailwind 클래스(`font-normal`·`font-bold`·`font-extrabold`)로 고릅니다.
...
`:has(#portal-root)` 로 한정하는 이유는 폰 화면(`/m/...`)이 px 고정값과 rem 이 섞여 있어
함께 키우면 레이아웃이 깨지기 때문입니다.
...
| `light` (설렘) | day | golden |
| `calm` (여유) | golden | day |
| `bold` (자신감) | night | golden |

**여행 → 공간 유형 우선순위:**

| 여행 | 1순위 | 2순위 |
|---|---|---|
| `explore` (도시 곳곳) | street | culture |
| `culture` (쇼핑·문화) | culture | street |
| `relax` (여유롭게) | leisure | culture |
...
각 World의 세 축 점수를 합산하고 최고점 World가 선택됩니다 (최대 7점).

| 축 | 1순위 일치 | 2순위 일치 |
|---|---|---|
| `timeOfDay` (무드가 결정) | +4 | +2 |
| `sceneType` (여행 스타일이 결정) | +2 | +1 |

여기에 컬러웨이 보정 **+1** 이 붙습니다 — `pink` + 어두운 World(`textOn: "light"`),
또는 `beige` + 밝은 World(`textOn: "dark"`) 일 때.

시간대 축 가중치가 높은 것은 무드가 체험의 전체 톤을 정하고 공간 축은 같은 시간대 안에서
도시를 세분화하는 역할이기 때문입니다. 동점이면 `ACTIVE_WORLD_IDS` 순서상 앞선 World 가
선택되며, 난수를 쓰지 않으므로 **같은 선택은 항상 같은 결과**가 나옵니다.
...
인물의 몸에 실제로 얹힌 것이라, 경계를 깎아도(`edgeShrink`) 그 픽셀은 알파 1 이라 그대로
남습니다. `spill` 만 값에 비례해 초록기를 줄이며 `1.0` 에서 완전히 사라지므로 기본값이
`1.0` 입니다. `min(g, (r+b)/2)` 가드 덕분에 초록기가 없는 색(피부·핑크·흰색·검정·데님)은
건드리지 않습니다.
...
부스 WiFi 가 끊기거나 서버가 죽어도 재촬영 전에는 업로드하지 않습니다. 사용자가 사진을
확정한 뒤 저장을 시도하고, 실패하면 같은 세션으로 재시도하거나 QR 없이 종료할 수 있습니다.
...
- 업로드에 끝내 실패하면 모바일 QR 없이 체험을 종료해야 합니다.
...
④ 나머지 → 여유** 입니다. ②가 ③보다 먼저인 것은 베이지·아이보리가 밝아도 여유로 붙잡히게
하기 위함이고, ③이 AND 가 아닌 OR 인 것은 파스텔(밝고 저채도)과 비비드 블루(어둡고 고채도)를
둘 다 잡기 위함입니다. 명도·채도는 HSL 의 `L`/`S` 가 아니라 BT.601 체감 밝기와
순색도(`delta/max`)로 잽니다 — HSL 은 고채도 색의 명도를 누르고 저채도 색의 채도를 부풀립니다.
...
04 에서 의류의 위치·크기를 안내하며 자동 감지하고, 15초 안에 조건을 맞추지 못하면 현재
프레임으로 계속할 수 있습니다. 05 에서 무드 판정(API 호출)을 실행합니다. World 매칭도 배경
...
하는 일이 달라 티어도 다르게 골랐습니다. **이 조합은 확정이며 연출 시간을 늘리려고 상위
모델로 바꾸지 마세요** — 체감 시간은 `OPENING_STAGES` 로만 조절합니다.

| 라우트 | 모델 | 이유 |
|---|---|---|
| `/api/analyze-mood` | **`gpt-5.6-luna`**, `reasoning_effort: "none"` | 의상 색 3분류는 Luna 의 설계 목적인 "단순 분류"에 맞습니다. 실측 1.8~3.1초 |
| `/api/passport` | **`gpt-5.6-terra`**, `reasoning_effort: "low"` | 제약(14~24자·어미) 있는 한국어 창작이라 상위 티어가 값을 합니다. 체험당 1회 호출이라 단가 차이는 무시할 수준 |
...
QR로 이어지는 모바일 결과 페이지를 연결합니다. 백엔드가 꺼져 있으면 재시도 또는
QR 없는 종료를 안내하며, 서버에 없는 세션 주소는 노출하지 않습니다.

- 09에서 사용자가 **이 사진 사용**으로 확정한 합성 JPEG + 선택값만 `POST /api/v1/sessions` 로 전송
- 응답으로 받은 `shareUrl` 로만 QR 생성 (업로드 실패 시 존재하지 않는 임시 QR은 표시하지 않음)
...
| `components/StepMoment.tsx` | 재촬영·사진 확정 후 서버 업로드 및 `shareUrl` 저장 |
...
`/api/passport` 가 즉석에서 만들어 표시할 뿐 백엔드로 전송하지도 세션에 저장하지도 않으므로
(API 응답에 해당 필드 없음), 부스 화면을 벗어나면 다시 볼 수 없습니다 — 의도된 동작입니다.
모바일 사진 페이지(`/m/{sessionId}/photo`)에도 여권 카드는 없습니다.
