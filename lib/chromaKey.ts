...
export interface ChromaKeyParams {
  ...
  keyColor: string;
  /**
   * 배경으로 간주할 CbCr 거리 상한. 실용 범위는 **0.05 ~ 0.30** — 무채색은 키 컬러에서
   * 약 0.33, 살색은 약 0.42 떨어져 있어 0.3 을 넘기면 사람까지 지워집니다.
   */
  similarity: number;
  /** 경계가 알파 0→1 로 넘어가는 폭 (CbCr 거리 단위). */
  smoothness: number;
  /**
   * 매트를 안쪽으로 깎는 정도 (0~1). 크로마 서브샘플링 탓에 생기는 반투명 경계 띠를
   * 잘라냅니다. **몸 윤곽의 초록 테두리에는 효과가 없습니다** — 그건 `spill` 담당.
   */
  edgeShrink: number;
  /**
   * 인물에 반사된 초록빛(스필) 제거 강도 (0~1). **몸 윤곽의 초록 테두리를 없애는 유일한
   * 노브입니다.** 인물 내부에 적용되는 최소치이고, 반투명한 경계 픽셀에는 알파에 비례해
   * 최대 1.0 까지 자동으로 걸립니다.
   */
  spill: number;
}
...
// 색 변환 — 아래 GLSL 과 **같은 수식**입니다. 04 무드 분석의 그린 가드가 재사용하므로
// 한쪽만 고치면 화면과 판정이 어긋납니다.
...
// cover 크롭을 UV 로 처리합니다. u_uvScale/u_uvOffset 이 coverRect() 결과라서
// 세그멘테이션 경로의 drawCover 와 프레이밍이 같습니다 (모드를 오가도 화각이 안 튑니다).
const VERTEX_SRC = `
...
void main() {
  ...
  // ② 스필 제거 — 초록이 r/b 평균보다 튀는 픽셀만 끌어내립니다. min() 덕분에 초록기가
  //    없는 픽셀은 그대로라 피부·소품 색이 상하지 않습니다. 몸 윤곽의 초록 테두리는
  //    알파 1 픽셀에 얹힌 색이라 ①이 아니라 여기서만 사라집니다.
...
/** 실패 시 throw 하지 않고 null — 호출자가 세그멘테이션으로 조용히 내려가야 합니다. */
export function createChromaKeyRenderer(
  canvas: HTMLCanvasElement,
  initialParams: ChromaKeyParams
): ChromaKeyRenderer | null {
  // premultipliedAlpha: false — true 면 drawImage 합성 시 색이 알파만큼 어두워집니다.
...
// 아이드로퍼 — /calibrate 와 07 튜닝 패널의 "화면에서 색 찍기".

let scratchCanvas: HTMLCanvasElement | null = null;

/**
 * 비디오 **원본 프레임**의 한 점 색을 "#rrggbb" 로 읽습니다. 합성 결과에는 이미 스필
 * 제거가 들어가 있어 그 색을 키 컬러로 쓰면 값이 밀립니다. 센서 노이즈를 피하려고
 * 주변 patch 를 평균냅니다.
 *
