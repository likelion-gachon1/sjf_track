  ...
  return (
    <button
      ...
      className={[
        SIZE_CLASSES[size],
        textColor,
        "relative flex w-full flex-col justify-end overflow-hidden text-left transition-transform duration-300",
        isInteractive ? "cursor-pointer hover:scale-[1.015]" : "cursor-default",
        selected ? "ring-2 ring-accent ring-offset-2 ring-offset-paper" : "",
        className,
      ].join(" ")}
      style={{ backgroundImage: world.gradient }}
    >
      {size !== "xs" && (
        <div>
      ...
      {size === "xs" && (
        <p className="font-serif text-sm leading-tight">{world.name}</p>
      )}
      {children}
    </button>
  );
